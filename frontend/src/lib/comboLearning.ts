...
/**
 * 「作曲」と「Claude作曲」は選択項目(GenerationInput)が全く同じで、表す創作の好み自体は
 * 変わらない(最終的にどちらのツールが文章を書いたかが違うだけ)ため、お気に入り組み合わせは
 * 両方の履歴をまとめて集計する。
 */
export function computeFavoriteCompositionCombos(
  entries: (CompositionHistoryEntry | ClaudeCompositionHistoryEntry)[],
): FavoriteCombo<GenerationInput>[] {
