...
import { excludeAutoGeneratedEntries } from './trainingData'
...
export function computeScoresByCategory(entries: HistoryEntry[]): Record<RankingCategory, Map<string, OptionScore>> {
  ...
  for (const entry of ratedEntries(excludeAutoGeneratedEntries(entries))) {
    ...
    if (entry.kind === 'composition' || entry.kind === 'claudeComposition') {
